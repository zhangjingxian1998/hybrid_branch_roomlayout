...
class NYU303(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        ret = {'img': img, 'plane_hm': hm, 'reg_mask': reg_mask, 'ind': ind, 'plane_wh': wh,
               'plane_offset': reg,
               'params3d': params3d,'img_name':crop_img}
        ...
        oseg_show = np.array(oseg,dtype=np.float32)
        oseg_show = oseg_show/np.max(oseg_show)*255
        oseg_show = np.array(oseg_show,dtype=np.uint8)
        ret['oseg'] = np.array(oseg)
        ret['oxy1map'] = oxy1map
