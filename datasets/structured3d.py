...
from models.utils import draw_umich_gaussian, gaussian_radius, line_gaussian, line_gaussian_horizen
...
class Structured3D(data.Dataset):
    def __init__(self, config, phase='training'):
        ...
        self.K = np.array([[762, 0, 640], [0, -762, 360],# 这个内参是从哪里来的？
    ...
    def __getitem__(self, item):
        ...
        if not os.path.exists(img_name):
            ret = {'img': img_name, 'plane_hm': 0, 'reg_mask': 0, 'ind': 0, 'plane_wh': 0, 'plane_offset': 0,
               'params3d': 0}
            return ret
        ...
        img=Image.fromarray(img)    # 部分图片直接使用Image读取会有通道错误
        ...
        pparams, labels, segs, endpoints = self.dataload(
            layout_name, sample[1],sample[3],sample[4], ratio_h, inh, inw)
        ...
        ret = {'img': img, 'plane_hm': hm, 'reg_mask': reg_mask, 'ind': ind, 'plane_wh': wh, 'plane_offset': reg,
               'params3d': params3d,'img_name':img_name}
