...
def _transpose_and_gather_feat(feat, ind):
    feat = feat.permute(0, 2, 3, 1).contiguous() # contiguous() 使得permute后的tensor数组不仅在语义上相连，另外开辟了一个内存空间使得内存上连续
    feat = feat.view(feat.size(0), -1, feat.size(3)) # [1,96*160,2]
...
def _neg_loss_gai(pred, gt, alpha=2, beta=4,gama=0.27):
    ''' Modified focal loss. Exactly the same as CornerNet.
        Runs faster and costs a little bit more memory
      Arguments:
        pred (batch x c x h x w)
    ...
    pos_neg_inds = torch.eq(gt.ne(1),gt.ne(0)).float()
    ...
    # neg_loss = torch.log(1 - pred) * torch.pow(pred,
    #                                            alpha) * neg_weights * neg_inds
    neg_loss = torch.log(1 - pred) * torch.pow(pred,alpha) * neg_inds
    pos_neg_loss = torch.log(1 - pred) * torch.pow(pred,alpha) * neg_weights * pos_neg_inds


    num_pos = pos_inds.float().sum()

    pos_loss = (1-gama)*(pos_loss.sum())
    neg_loss = gama*(neg_loss.sum())
    pos_neg_loss = (1-gama)*(pos_neg_loss.sum())

    if num_pos == 0:
        loss = loss - neg_loss
    else:
        loss = loss - (pos_loss + neg_loss + pos_neg_loss) / num_pos
    return loss
...
class FocalLoss_gai(pl.LightningModule):
    '''nn.Module warpper for focal loss'''

    def __init__(self):
        super(FocalLoss_gai, self).__init__()
        self.neg_loss = _neg_loss_gai

    def forward(self, out, target, alpha=2, beta=4):
        return self.neg_loss(out, target, alpha, beta)

class RegL1Loss(pl.LightningModule):  # L1 loss
    def __init__(self):
...
class ParseL1loss(pl.LightningModule):
    def __init__(self):
...
class DepthLoss(pl.LightningModule):
    ...
    def InverDepthByInstance(self, output, mask, ind, seg, K_inv, xy1):
        ...
        for i in range(b):
            ...
            index[index == -1] = 0
            # if res.size()[0]==0:
            #     res = torch.zeros(int(torch.max(index.long()))+1,96,160).to('cuda')
...
class Loss(pl.LightningModule):
    ...
    def forward(self, outputs, **batch):
        ...
        loss = self.w_pcenter * plane_hm_loss + self.w_psize * plane_wh_loss + \
            ...
            self.w_lregion * line_hm_loss + self.w_loffset * line_offset_loss + self.w_lalpha * line_alpha_loss
